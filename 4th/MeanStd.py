...
def get_image_stats(folder_path):
    # 하위 폴더에 있는 모든 이미지들 들고옴
    # 확장자 jpg, jpeg 조건을 줘야함
    image_paths = glob.glob(f"{folder_path}/**/*", recursive=True)  # 재귀적으로 폴더 구조 구석구석 탐색
    valid_extensions = (".png", "jpg", "jpeg")
    all_images = [img for img in image_paths if img.lower().endswith(valid_extensions)]  # 지정된 확장자로 끝나면
