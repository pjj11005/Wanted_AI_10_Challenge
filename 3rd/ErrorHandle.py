...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 10. cpu, cuda 위치 바꿈
...
myMLP = MLP(image_size, hidden_size, num_classes).to(device)  # 11. 모델을 gpu 위에 올려야 함
...
train_loader = DataLoader(dataset=train_mnist, batch_size=batch_size, shuffle=True)  # 2. batch_size 변수가 str이었음
test_loader = DataLoader(dataset=test_mnist, batch_size=batch_size, shuffle=True)

loss_fn = nn.CrossEntropyLoss()

optim = Adam(params=myMLP.parameters(), lr=lr)  # 3. str -> float / 4. parameters -> parameters()
...
for epoch in range(total_epochs):
    for idx, (image, label) in enumerate(train_loader):
        ...
        loss = loss_fn(output, label)  # 6. Loss 객체 선언 해야함 / 7. output shape에 이상한 값 들어감
